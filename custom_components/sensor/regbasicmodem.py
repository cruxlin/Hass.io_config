...
class BasicModem(object):
    ...
    def __init__(self, port=DEFAULT_PORT, incomingcallback=None):
        ...
        try:
            """ Reg modified to configure serial port as 115200 8N1 CTS/RTS flow control"""
            self.ser = serial.Serial(port=self.port,baudrate=115200,timeout=1, rtscts=1)  
    ...
    def _modem_sm(self):
        ...
        while self.ser:
            ...
            """Reg modified to support Taiwanese DTMF caller ID type"""
            if cid_field in ['DTMF']:  
                """ abort if cid_data does not meet the length of phone number in Taiwan """
                if len(cid_data) <= 6: 
                   continue
                self.cid_number = cid_data
                self.cid_time = datetime.datetime.now()
