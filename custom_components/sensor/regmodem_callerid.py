...
20170919 rename to regmodem_callerid and replace module "basicmodem" to customized "regbasicmodem"
"""
import logging
...
""" remove official basicmodem module """

_LOGGER = logging.getLogger(__name__)
DEFAULT_NAME = 'RegModem CallerID'
...
def setup_platform(hass, config, add_devices, discovery_info=None):
    ...
    """use local customized basicmodem module """
    from .regbasicmodem import BasicModem as bm  
    """from basicmodem import BasicModem as bm"""
...
class ModemCalleridSensor(Entity):
    ...
    def _incomingcallcallback(self, newstate):
        """Handle new states."""
        _LOGGER.debug('new state: %s',newstate)
        if newstate == self.modem.STATE_RING:
            ...
            _LOGGER.debug('cid_number: %s',self.modem.get_cidnumber)
            self.schedule_update_ha_state()
